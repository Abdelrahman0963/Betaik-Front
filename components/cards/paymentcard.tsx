...
interface Props {
    ...
    checked?: boolean;
    onCheckChange?: (val: boolean, plan: PaymentPlanType) => void;
    ...
    return (
        <div className="w-full min-h-[360px] rounded-[10px] px-6 py-5 border border-gray-200 flex flex-col gap-4 relative">
            ...
            <div className="flex justify-between items-start">
                ...
                {/* status */}
                {data.status === "active" ? (
                    <div className="px-3 h-6 rounded-2xl bg-[#ABEFC6] text-[#067647] text-[14px] flex items-center justify-center">
                        Active
                    </div>
                ) : (
                    <div className="px-3 h-6 rounded-2xl bg-[#F3F4F6] text-[#606777] text-[14px] flex items-center justify-center">
                        Draft
                    </div>
                )}
                ...
                {/* checkbox */}
                {showCheckbox && (
                    <div className="absolute top-3 right-3 bg-white/80 p-2 rounded-full flex items-center justify-center">
                        <input
                            type="checkbox"
                            checked={isChecked}
                ...
                {!showCheckbox && (
                    <div className="relative">
                        <HiOutlineDotsVertical
                            className="text-[#7F8595] text-[20px] cursor-pointer"
                            onClick={() =>
                        ...
                        {showActions === index && (
                            <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-md z-20 p-3">
                                ...
                                <div
                                    ...
                                    className="flex items-center gap-3 px-2 py-2 rounded-md hover:bg-gray-100 cursor-pointer text-[#DC2626]"
                                >
                                    <FaRegTrashAlt />
                                    <span>Delete</span>
                                </div>
            ...
            {/* title */}
            <div className="text-[16px] font-medium text-[#191B1F]">
                {data.planName}
            </div>
            ...
            <div className="flex flex-col gap-3 rounded-xl p-4 bg-[#D0DFFE]">
                ...
                <div className="flex justify-between">
                    <div className="text-[14px] text-[#414A5B]">
                        Payment Frequency:
                    </div>
