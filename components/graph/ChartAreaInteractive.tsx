...
export function ChartAreaInteractive() {
    ...
    const [adminTimeRange, setAdminTimeRange] = React.useState("3m") // for Admin buttons
    ...
    const filteredData = chartData.filter((item) => {
        ...
        if (user?.role?.toLowerCase() === "superadmin") {
            // map admin button ranges
            if (adminTimeRange === "3m") daysToSubtract = 90
            else if (adminTimeRange === "6m") daysToSubtract = 180
            else if (adminTimeRange === "12m") daysToSubtract = 365
        } else {
            // client select ranges
    ...
    const showUniversity = role === "university"
    const showDeveloper = role === "developer"
                ...
                {showDeveloper && (
                    <>
                        <div className="grid flex-1 gap-1">
                            ...
                            <CardDescription>
                                Track property engagement over time
                            </CardDescription>
                        </div>
                        <Select value={timeRange} onValueChange={setTimeRange}>
                            <SelectTrigger
                ...
                {showAdmin && (
                    <>
                        <div className="grid flex-1 gap-1">
                            ...
                            <CardDescription>
                                Total, new, and returning users over time
                            </CardDescription>
                        </div>
                        <ButtonGroup>
                            {["3m", "6m", "12m"].map((range) => (
                                <Button
                                    key={range}
                                    className={`cursor-pointer border border-gray-200  bg-blue-100/10 py-4 ${adminTimeRange === range ? " text-black hover:bg-blue-100/40 bg-blue-100/40 " : " text-gray-700 hover:bg-blue-100/10"}`}
                                    size="sm"
                                    onClick={() => setAdminTimeRange(range)}
                                >
                    ...
                    <AreaChart data={filteredData}>
                        <defs>
                            <linearGradient id="fillDesktop" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="#7a73f5" stopOpacity={0.8} />
                                <stop offset="95%" stopColor="#7a73f5" stopOpacity={0.1} />
                            </linearGradient>
                            <linearGradient id="fillMobile" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="#a1bfff" stopOpacity={0.8} />
                                <stop offset="95%" stopColor="#a1bfff" stopOpacity={0.1} />
                            </linearGradient>
                            <linearGradient id="fillTablet" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="#145efc" stopOpacity={0.8} />
                                <stop offset="95%" stopColor="#145efc" stopOpacity={0.1} />
                            </linearGradient>
                        </defs>
                        ...
                        />
                        <Area dataKey="Views" type="natural" fill="url(#fillMobile)" stroke="#145efc" stackId="a" />
                        <Area dataKey="Compares" type="natural" fill="url(#fillDesktop)" stroke="#7a73f5" stackId="a" />
                        <Area dataKey="Saves" type="natural" fill="url(#fillTablet)" stroke="#a1bfff" stackId="a" />
