    ...
    return (
        <div className="px-4 sm:px-6 lg:px-0">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="text-sm md:text-base font-medium text-[#414A5B]">
                    Manage all payment plans and special offers for your dorms.
                </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-5">
                    ...
                    <div className="flex flex-wrap gap-4 md:gap-5">
                        {["Active", "Draft", "Expired"].map((tab) => (
                            <div
                                key={tab}
                                className={`cursor-pointer text-sm md:text-base ${activeTab === tab ? "text-[#155DFC]" : "text-[#7F8595]"}`}
                    ...
                    {/* Button */}
                    <div
                        className="cursor-pointer w-full md:w-auto h-11 md:h-12 items-center justify-center flex text-white gap-2 px-4 md:px-6 bg-[#155DFC] rounded-xl hover:bg-[#0F4BD8]"
            ...
            {/* Filters */}
            <div className="flex flex-col md:flex-row md:flex-wrap gap-4 mt-9">

                {/* Search */}
                <div className="w-full md:flex-1 h-11 items-center flex gap-2.5 border border-gray-200 rounded-[10px]">
                    <div className="ml-4">
                ...
                {["All Statuses", "All Dorms"].map((text) => (
                    <div
                        key={text}
                        className="flex justify-between w-full md:w-[220px] lg:w-[260px] h-11 items-center border border-gray-200 rounded-[10px]"
                    >
                        <div className="text-[14px] ml-4 text-[#7F8595]">
                            {text}
                        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 mt-8">
                {cards.map((c, index) =>
                    c.type === "empty" ? (
                        <div
                            key={index}
                            className="w-full min-h-[300px] rounded-[10px] border-2 border-dashed border-[#155DFC] px-6 py-5 flex flex-col gap-2 justify-center items-center"
                        >
                            <div className="w-12 h-12 rounded-[10px] bg-[#F2F2FE] flex items-center justify-center">
                                <FaPlus
                                    onClick={() => router.push("/payment/paymentform")}
                                    className="text-[#7A73F4] text-[22px] cursor-pointer hover:text-[#7A9DE0]"
                                />
                            </div>

                            <div className="mt-3 text-[16px] font-medium text-[#191B1F]">
                                Create New Listing
                            </div>

                            <div className="text-[14px] text-[#606777] text-center">
                                Add a property to publish
                            </div>

                            <div className="w-full h-10 bg-[#8AAEFE] py-2 rounded-[10px] text-center cursor-pointer text-white hover:bg-[#7A9DE0]">
                                Post Property
                            </div>
                        </div>
                    ) : (
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50 px-4">
                    <div className="bg-white p-6 w-full max-w-[384px] rounded-3xl flex flex-col justify-center items-center gap-4">
                        ...
                        <div className="text-center text-[#4A5565] text-sm md:text-base">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full mt-3">
                            <button
                                ...
                                className="w-full sm:w-1/2 px-4 py-2 bg-[#D1D5DC] rounded-[14px]"
                            >
                                Cancel
                            </button>
                            ...
                            <button
                                ...
                                className="w-full sm:w-1/2 px-4 py-2 bg-[#FB2C36] text-white rounded-[14px]"
                            >
                                Delete Offer
                            </button>
                        </div>
