    ...
    return (
        <div className="px-4 sm:px-6 lg:px-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 mt-8">
                {cards.map((c, index) =>
                    c.type === "empty" ? (
                        <div key={index} className="w-full min-h-[360px] rounded-[10px] border-2 border-dashed border-[#155DFC] px-6 py-5 flex flex-col gap-2 justify-center items-center">
                            <div className="w-12 h-12 rounded-[10px] bg-[#F2F2FE] flex items-center justify-center">
                                <FaPlus onClick={() => router.push("/paymentform")} className="text-[#7A73F4] text-[22px] cursor-pointer hover:text-[#7A9DE0] transition-colors duration-200" />
                            </div>
                            ...
                            <div className="w-full h-10 bg-[#8AAEFE] py-2 rounded-[10px] text-center cursor-pointer text-white hover:bg-[#7A9DE0] transition-colors duration-200">Post Property</div>
                        </div>
                    ) : (
                        <div key={index} className="w-full min-h-[360px] rounded-[10px] px-6 py-5 border border-gray-200 flex flex-col gap-4 relative">
                            <div className="flex justify-between">
                                <div>
                                    {c.data.status === "active" ? (
                                        <div className="px-3 h-6 rounded-2xl bg-[#ABEFC6] text-[#067647] text-center text-[14px] flex items-center justify-center">Active</div>
                                    ) : (
                                        <div className="px-3 h-6 rounded-2xl bg-[#F3F4F6] text-[#606777] text-center text-[14px] flex items-center justify-center">Draft</div>
                                    )}
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    {showActions === index && (
                                        <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-md z-20 p-3">
                                            <div onClick={() => handleEdit(c.data)} className="flex items-center gap-3 px-2 py-2 rounded-md hover:bg-gray-100 cursor-pointer">
                                                <FaRegEdit />
                                                <span>Edit</span>
                                            </div>
                            ...
                            <div className="text-[16px] font-medium text-[#191B1F]">{c.data.planName}</div>
                            ...
                            <div className="flex flex-col gap-3 rounded-xl p-4 bg-[#D0DFFE]">
                                <div className="flex justify-between">
                                    ...
                                    <div className="font-medium">{c.data.price}</div>
                                </div>

                                <div className="flex justify-between">
                                    <div className="text-[14px] text-[#414A5B]">Payment Frequency:</div>
                                    <div className="text-[#447DFD]">{c.data.frequency}</div>
                                </div>

                                <div className="flex justify-between">
                                    <div className="text-[14px] text-[#414A5B]">Installments:</div>
                                    <div className="font-medium">{c.data.installments}</div>
                                </div>
                            </div>
